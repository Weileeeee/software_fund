...
window.markSafe = function () {
    // Use self_safe endpoint which uses current session user
    fetch('/self_safe', { method: 'POST' })
        .then(response => {
            if (response.ok) {
                ...
                // OPTION A: Simple reload (Easiest)
                window.location.reload(); 
                
                // OPTION B: If you have a specific function to refresh the table
                // updateDashboardData(); 
            } else {
                alert("Failed to update status. Please try again.");
            }
        })
        .catch(err => console.error("Error updating safety status:", err));
};
    ...
    let markers = []; // User's custom marker for reporting
    let reportLat = 0;
    let reportLng = 0;
    ...
    if (incidentForm) {
        incidentForm.addEventListener("submit", function (e) {
            ...
            const params = `type=${formData.get('type')}&location=${formData.get('location')}&time=${formData.get('time')}&lat=${lat}&lng=${lng}&description=${desc}&severity=${sev}&evidence=${evid}`;
        ...
        fetch('/get_incidents')
            ...
            .then(data => {
                ...
                rows.forEach(r => {
                    ...
                    const [id, type, loc, time, status, reporter, lat, lng] = r.split('|');
