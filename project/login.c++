...
string SaveIncident(SQLHDBC sqlConnHandle, string type, string loc, string time, string lat, string lng, string desc, string sev, string evid, string reporter) {
    ...
    type = clean(type); loc = clean(loc); desc = clean(desc); sev = clean(sev); evid = clean(evid);
    
    string query = "INSERT INTO Incidents (incident_type, location, incident_time, latitude, longitude, description, severity, evidence_path, status, reported_by) VALUES ('" 
...
string GetAllIncidents(SQLHDBC sqlConnHandle) {
    SQLHSTMT hStmt;
    SQLAllocHandle(SQL_HANDLE_STMT, sqlConnHandle, &hStmt);
    string query = "SELECT incident_id, incident_type, location, DATE_FORMAT(incident_time, '%Y-%m-%d %H:%i'), status, reported_by FROM Incidents ORDER BY incident_time DESC;";
    SQLExecDirectA(hStmt, (SQLCHAR*)query.c_str(), SQL_NTS);
    
    string result = "";
    char id[20], type[100], loc[100], time[50], stat[50], rep[100];
    while (SQLFetch(hStmt) == SQL_SUCCESS) {
        SQLGetData(hStmt, 1, SQL_C_CHAR, id, sizeof(id), NULL);
        ...
        result += string(id) + "|" + type + "|" + loc + "|" + time + "|" + stat + "|" + rep + ";";
    }
    SQLFreeHandle(SQL_HANDLE_STMT, hStmt);
    return result;
}

bool UpdateIncidentStatus(SQLHDBC sqlConnHandle, string incidentId, string status) {
    SQLHSTMT hStmt;
    SQLAllocHandle(SQL_HANDLE_STMT, sqlConnHandle, &hStmt);
    string query = "UPDATE Incidents SET status = '" + status + "' WHERE incident_id = " + incidentId + ";";
    bool success = SQL_SUCCEEDED(SQLExecDirectA(hStmt, (SQLCHAR*)query.c_str(), SQL_NTS));
    SQLFreeHandle(SQL_HANDLE_STMT, hStmt);
    return success;
}
