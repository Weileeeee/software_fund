...
bool StartEvacuation(SQLHDBC sqlConnHandle, string block) {
    ...
    // Mark ALL students as Missing when evacuation starts
    // Wardens must manually mark students as Safe during evacuation
    ...
    if (!missingSuccess) {
        cout << "[ERROR] Failed to mark students as Missing. Query: " << queryMissing << endl;
        return false;
    }
    
    cout << "[EVACUATION] Evacuation started for block: " << block << endl;
    cout << "[EVACUATION] All students marked as Missing - wardens must manually mark them Safe" << endl;
...
bool StartClassEvacuation(SQLHDBC sqlConnHandle, string classGroup) {
    ...
    // Mark all students in the class as "Missing" when evacuation starts
    // Lecturers can then manually update status to "Safe" or other statuses
    ...
    cout << "[EVACUATION] All students marked as 'Missing'" << endl;
    cout << "[EVACUATION] Lecturers can update status via dropdown" << endl;
    ...
    // 3. Prepare evacuation message
    string targetArea = (classGroup == "All") ? "ALL CLASSES" : ("Class " + classGroup);
    string evacuationMessage = "🚨 EMERGENCY EVACUATION ALERT 🚨\\n\\n"
                               "Evacuation initiated for " + targetArea + ".\\n"
                               "Please proceed to the nearest emergency assembly point immediately.\\n"
                               "Follow emergency protocols and await further instructions.\\n\\n"
    ...
    // 4. Send WhatsApp notifications to all students
    int successCount = 0;
    int failCount = 0;
    ...
    for (const auto& student : students) {
        string name = student.first;
        string phone = student.second;
        cout << "[SENDING] WhatsApp to " << name << " (" << phone << ")..." << endl;
        if (SendWhatsAppMessage(phone, evacuationMessage)) {
            successCount++;
        } else {
            failCount++;
        }
        Sleep(200); // Slight delay between messages
    }
...
bool MarkStudentSafe(SQLHDBC sqlConnHandle, string userId) {
    ...
    string query = "INSERT INTO evacuationlogs (user_id, status) VALUES (" + userId + ", 'Safe') "
                   "ON DUPLICATE KEY UPDATE status='Safe';";
    
    cout << "[DEBUG] Executing MarkStudentSafe: " << query << endl;
    ...
    if (!SQL_SUCCEEDED(SQLExecDirectA(hStmt, (SQLCHAR*)query.c_str(), SQL_NTS))) {
        ...
        cout << "[SQL ERROR] MarkStudentSafe Failed: " << message << endl;
        SQLFreeHandle(SQL_HANDLE_STMT, hStmt);
        return false;
    }
...
string GetBroadcasts(SQLHDBC sqlConnHandle) {
    ...
    if (!SQL_SUCCEEDED(SQLExecDirectA(hStmt, (SQLCHAR*)query.c_str(), SQL_NTS))) {
        cout << "[ERROR] Failed to fetch broadcasts." << endl;
        SQLCHAR sqlState[1024];
        SQLCHAR message[1024];
        SQLINTEGER nativeError;
        SQLSMALLINT textLength;
...
// Update student evacuation status
// Update student evacuation status
bool UpdateStudentEvacStatus(SQLHDBC sqlConnHandle, string studentId, string status) {
    SQLHSTMT hStmt;
    SQLAllocHandle(SQL_HANDLE_STMT, sqlConnHandle, &hStmt);
    
    // Validate status to prevent errors
    string cleanStatus = "Unknown";
    if (status == "Safe" || status == "Missing" || status == "Not Safe") cleanStatus = status;
    
    // Use ON DUPLICATE KEY UPDATE
    string query = "INSERT INTO evacuationlogs (user_id, status) VALUES (" + studentId + ", '" + cleanStatus + "') "
                   "ON DUPLICATE KEY UPDATE status='" + cleanStatus + "';";
...
int main() {
    ...
    if (!SQL_SUCCEEDED(SQLDriverConnectA(sqlConn, NULL, (SQLCHAR*)connStr.c_str(), SQL_NTS, NULL, 0, NULL, SQL_DRIVER_NOPROMPT))) {
        ...
        cout << "Native Error: " << nativeError << endl;
    ...
    while (true) {
        ...
        if (req.find("GET / ") != string::npos || req.find("GET /login.html") != string::npos) {
            ...
            string rs = "<script>window.SERVER_INJECTED_ROLE = '" + currentUserRole + "'; console.log('Injected Role:', window.SERVER_INJECTED_ROLE);</script>";
        ...
        // RESIDENCE MONITORING
            ...
            string userId = GetP(body, "id");
            string status = GetP(body, "status");
            
            cout << "[DEBUG] Updating stay status for user " << userId << " to " << status << endl;
            
            SQLHSTMT hStmt;
            SQLAllocHandle(SQL_HANDLE_STMT, sqlConn, &hStmt);
            
            // Update stay_status in users table
            string query = "UPDATE users SET stay_status = '" + status + "' WHERE user_id = " + userId + ";";
            
            if (SQL_SUCCEEDED(SQLExecDirectA(hStmt, (SQLCHAR*)query.c_str(), SQL_NTS))) {
                cout << "[SUCCESS] Stay status updated for user " << userId << endl;
                resp = "HTTP/1.1 200 OK\r\n\r\nUpdated";
            } else {
                cout << "[ERROR] Failed to update stay status for user " << userId << endl;
                SQLCHAR sqlState[1024];
                SQLCHAR message[1024];
                SQLINTEGER nativeError;
                SQLSMALLINT textLength;
        ...
        // --- LECTURER ROUTES ---
            ...
            if (SQL_SUCCEEDED(SQLExecDirectA(hStmt, (SQLCHAR*)q.c_str(), SQL_NTS))) {
                resp = "HTTP/1.1 200 OK\r\n\r\nSaved";
                cout << "[BROADCAST] Saved successfully." << endl;
            } else {
                resp = "HTTP/1.1 500 Error\r\n\r\nFail";
                cout << "[BROADCAST] Failed to save!" << endl;
                SQLCHAR sqlState[1024];
                SQLCHAR message[1024];
                SQLINTEGER nativeError;
                SQLSMALLINT textLength;
