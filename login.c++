...
const string DB_PASS = "Gyl2005."; // Update with your actual password
...
string GetResidentList(SQLHDBC sqlConnHandle, string block) {
    SQLHSTMT hStmt;
    SQLAllocHandle(SQL_HANDLE_STMT, sqlConnHandle, &hStmt);
    string query = "SELECT u.user_id, u.full_name, u.block, u.stay_status FROM Users u LEFT JOIN EvacuationLogs sp ON u.user_id = sp.user_id WHERE u.role IN ('Student', 'student') AND u.block = '" + block + "';";
    SQLExecDirectA(hStmt, (SQLCHAR*)query.c_str(), SQL_NTS);
    ...
    while (SQLFetch(hStmt) == SQL_SUCCESS) {
        ...
        result += string(id) + "|" + name + "|" + blk + "|" + status + ";";
    }
    SQLFreeHandle(SQL_HANDLE_STMT, hStmt);
    return result;
}

bool UpdateStayStatus(SQLHDBC sqlConnHandle, string userId, string status) {
    SQLHSTMT hStmt;
    SQLAllocHandle(SQL_HANDLE_STMT, sqlConnHandle, &hStmt);
    string query = "UPDATE studentprofiles SET stay_status = '" + status + "' WHERE user_id = " + userId + ";";
...
string SaveIncident(SQLHDBC sqlConnHandle, string type, string loc, string time, string lat, string lng, string desc, string sev, string evid, string reporter) {
    ...
    auto cleanTime = [](string s) { for(auto &c : s) if(c == 'T') c = ' '; return s; };
    type = clean(type); loc = clean(loc); time = cleanTime(time); desc = clean(desc); sev = clean(sev); evid = clean(evid);
    
    // FIXED COLUMN NAMES: location_name, reported_at, incident_status, reporter_name
    string query = "INSERT INTO Incidents (incident_type, location_name, reported_at, latitude, longitude, description, severity, evidence_path, incident_status, reporter_name) VALUES ('" 
                   + type + "', '" + loc + "', '" + time + "', '" + lat + "', '" + lng + "', '" + desc + "', '" + sev + "', '" + evid + "', 'Pending', '" + reporter + "');";
    ...
    if (SQL_SUCCEEDED(SQLExecDirectA(hStmt, (SQLCHAR*)query.c_str(), SQL_NTS))) {
        ...
        SQLSMALLINT textLen;
        SQLGetDiagRecA(SQL_HANDLE_STMT, hStmt, 1, sqlState, &nativeError, msg, sizeof(msg), &textLen);
...
string GetAllIncidents(SQLHDBC sqlConnHandle) {
    SQLHSTMT hStmt;
    SQLAllocHandle(SQL_HANDLE_STMT, sqlConnHandle, &hStmt);
    // FIXED COLUMN NAMES and Added Filter for Resolved (Rejected) incidents. Added latitude and longitude.
    string query = "SELECT incident_id, incident_type, location_name, DATE_FORMAT(reported_at, '%Y-%m-%d %H:%i'), incident_status, reporter_name, latitude, longitude FROM Incidents WHERE incident_status != 'Rejected' ORDER BY reported_at DESC;";
    SQLExecDirectA(hStmt, (SQLCHAR*)query.c_str(), SQL_NTS);
    
    string result = "";
    char id[20], type[100], loc[100], time[50], stat[50], rep[100], lat[20], lng[20];
    while (SQLFetch(hStmt) == SQL_SUCCESS) {
        SQLGetData(hStmt, 1, SQL_C_CHAR, id, sizeof(id), NULL);
        ...
        result += string(id) + "|" + type + "|" + loc + "|" + time + "|" + stat + "|" + rep + "|" + lat + "|" + lng + ";";
    }
    SQLFreeHandle(SQL_HANDLE_STMT, hStmt);
    return result;
}
...
string GetEvacuationList(SQLHDBC sqlConnHandle) {
    SQLHSTMT hStmt;
    SQLAllocHandle(SQL_HANDLE_STMT, sqlConnHandle, &hStmt);
    string query = "SELECT u.user_id, u.full_name, u.block, IFNULL(sp.status, 'Unknown') FROM Users u LEFT JOIN EvacuationLogs sp ON u.user_id = sp.user_id WHERE u.role IN ('Student', 'student') ORDER BY u.block;";
    SQLExecDirectA(hStmt, (SQLCHAR*)query.c_str(), SQL_NTS);
...
vector<pair<string, string>> GetStudentsInBlock(SQLHDBC sqlConnHandle, string block) {
    ...
    string query = "SELECT u.full_name, IFNULL(u.phone_number, 'No Phone') FROM Users u "
                   " LEFT JOIN EvacuationLogs sp ON u.user_id = sp.user_id "
                   "WHERE u.role IN ('Student', 'student') AND u.block = '" + block + "';";
...
bool StartEvacuation(SQLHDBC sqlConnHandle, string block) {
    ...
    // 1. Reset all students' safety status in the specified block(s)
    // NOTE: 'Not Safe' is not in the ENUM list. Using 'Missing' as the appropriate status for unsafe/not-checked-in.
    string query = "UPDATE EvacuationLogs sp JOIN Users u ON sp.user_id = u.user_id "
                   "SET sp.status = 'Missing' WHERE u.role IN ('Student', 'student')" + blockFilter + ";";
    
    bool dbSuccess = SQL_SUCCEEDED(SQLExecDirectA(hStmt, (SQLCHAR*)query.c_str(), SQL_NTS));
    SQLFreeHandle(SQL_HANDLE_STMT, hStmt);
    
    if (!dbSuccess) {
        cout << "[ERROR] Failed to update safety status in database. Query: " << query << endl;
        return false;
    }
    ...
    // specific GetStudentsInBlock uses strict equality, we need a flexible query here.
    // So we'll inline the logic or modify GetStudentsInBlock. Inlining for safety/speed.
    vector<pair<string, string>> students; 
    SQLAllocHandle(SQL_HANDLE_STMT, sqlConnHandle, &hStmt);
    
    string selectQuery = "SELECT u.full_name, IFNULL(u.phone_number, 'No Phone') FROM Users u "
                         "LEFT JOIN EvacuationLogs sp ON u.user_id = sp.user_id "
                         "WHERE u.role IN ('Student', 'student') " + blockFilter + ";";
...
void GenerateUnsafeReport(SQLHDBC sqlConnHandle) {
    SQLHSTMT hStmt;
    SQLAllocHandle(SQL_HANDLE_STMT, sqlConnHandle, &hStmt);
    string query = "SELECT u.full_name, u.block, IFNULL(u.phone_number, 'No Phone') FROM Users u "
                   "JOIN EvacuationLogs sp ON u.user_id = sp.user_id "
                   "WHERE sp.status = 'Missing' AND u.role IN ('Student', 'student');";
    SQLExecDirectA(hStmt, (SQLCHAR*)query.c_str(), SQL_NTS);
...
string GetClassStudents(SQLHDBC sqlConnHandle) {
    SQLHSTMT hStmt;
    SQLAllocHandle(SQL_HANDLE_STMT, sqlConnHandle, &hStmt);
    string query = "SELECT sp.user_id, u.full_name, u.block, sp.status FROM EvacuationLogs sp JOIN Users u ON sp.user_id = u.user_id WHERE u.role IN ('Student', 'student');";
    SQLExecDirectA(hStmt, (SQLCHAR*)query.c_str(), SQL_NTS);
...
int main() {
    ...
    while (true) {
        ...
        // --- LECTURER ROUTES ---
            ...
            string q = "UPDATE EvacuationLogs el JOIN Users u ON el.user_id = u.user_id SET el.status='" + stat + "' WHERE u.student_code='" + sid + "'";
            SQLExecDirectA(hStmt, (SQLCHAR*)q.c_str(), SQL_NTS);
            SQLFreeHandle(SQL_HANDLE_STMT, hStmt);
            resp = "HTTP/1.1 200 OK\r\n\r\nUpdated";
        }
